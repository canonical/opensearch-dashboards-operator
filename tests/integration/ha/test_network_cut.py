...
async def extra_secure_wait_for_idle(ops_test: OpsTest):
    ...
    """
    num_app_units = len(ops_test.model.applications[APP_NAME].units)
    num_tls_units = len(ops_test.model.applications[TLS_CERTIFICATES_APP_NAME].units)
    await ops_test.model.wait_for_idle(
        apps=[APP_NAME],
        status="active",
        timeout=3000,
        idle_period=30,
        wait_for_at_least_units=num_app_units,
    )
    await ops_test.model.wait_for_idle(
        apps=[APP_NAME], status="active", timeout=1000, wait_for_at_least_units=num_tls_units
    )
    await ops_test.model.wait_for_idle(
        apps=[APP_NAME],
        status="active",
...
async def test_deploy_active(ops_test: OpsTest):
    ...
    await ops_test.model.deploy(
        TLS_CERTIFICATES_APP_NAME, channel="stable", config=config, revision=136
    )
...
async def test_set_tls(ops_test: OpsTest, request):
    ...
    await ops_test.model.relate(TLS_CERTIFICATES_APP_NAME, APP_NAME)
    await asyncio.sleep(RESTART_DELAY * 2)
    await extra_secure_wait_for_idle(ops_test)
